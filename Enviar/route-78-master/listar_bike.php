	...
	if($qtd > 0){
		...
		while($row = $result->fetch_assoc()){	
			...
			<button class='btn btn-success' onclick=\"location.href='pedido.php?acao=add&id=".$row["codigo_produto"]."';\"><i class='fa fa-add'></i></button>
					...
					<button class='btn btn-success' onclick=\"location.href='index2.php?page=edit_bike&cod_bike=".$row["codigo_produto"]."';\"><i class='fa fa-edit'></i></button>
					
					<button class='btn btn-danger' onclick=\"location.href='index2.php?page=salvar_bike&acao=excluir&cod_bike=".$row["codigo_produto"]."';\"><i class='fa fa-trash'></i></button>
